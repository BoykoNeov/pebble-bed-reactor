...
func _process(delta: float) -> bool:
	...
	# Catch a rider mid-ride and remember its state, to prove it is frozen in transit.
	if _t > 32.0 and _frozen_id == -1 and _main._loop.count() > 0:
		for id in _main._out_of_core:
			var peb = _main._pebbles.get(id)
			if peb != null and peb.burnup > 0.0:
				_frozen_id = id
	...
	# The freeze check: same pebble, still riding, must not have burned or cooled.
	if _frozen_id != -1 and not _checked_freeze and _main._out_of_core.has(_frozen_id):
		if _t >= 38.0:
			_checked_freeze = true
			var peb = _main._pebbles.get(_frozen_id)
			if peb != null:
				print("  rider #%d in transit: burnup %.4f -> %.4f   T %.1f -> %.1f K" % [
					_frozen_id, _frozen_burnup, peb.burnup, _frozen_temp, peb.temperature])
				...
				_check(absf(peb.temperature - _frozen_temp) < 1.0e-6,
					"a riding pebble's temperature is frozen for the ride")
